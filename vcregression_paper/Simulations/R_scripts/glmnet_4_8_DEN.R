...
mks = c(1, 24, 252, 1512, 5670, 13608, 20412, 17496, 6561)
nrep = 3
	...
	for (j in 1:3){
		for (i in 1:imax){
		  ...
		  if (j==1){
		    ...
		    save(cva_out, file = paste("out/cva", name, modelname, i, '.RData', sep="_"))
		    best.params = bestpara(cva_out)
		    pd <- predict(cva_out, design[,1: sum(mks[1:(k+1)])], alpha = best.params$alpha)
		    
		  } else {
		    load(paste("out/cva", name, modelname, i, '.RData', sep="_"))
		    best.params = bestpara(cva_out)
		    cvfit <- cv.glmnet(design[tr,1: sum(mks[1:(k+1)])], response[tr], alpha = best.params$alpha)
